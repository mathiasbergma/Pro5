...
\begin{DoxyEnumerate}
\item Call load\+Cert() to loads certificates from filesystem and upload to module (If using SSL/\+TLS). This function is called 3 times, once for each certificate (CA, CERT, KEY)
\end{DoxyEnumerate}
...
\end{DoxyParams}
\begin{DoxyReturn}{Returns}
-\/1 if load file fails, return value from set\+Cert\+MQTT if successful 
\end{DoxyReturn}
...
\end{DoxyParams}
\begin{DoxyReturn}{Returns}
0 if successful, 1 if not, -\/1 if memory allocation failed 
\end{DoxyReturn}
